...
[![Version 5.15.0-rc.6](https://img.shields.io/badge/version-5.15.0--rc.6-green.svg)](VERSION)
...
| 5.15.0-rc.2 | 2026-05-10 | **refactor(http+ssot): centralise HTTP-API plumbing and finish UTC + governance dedup.** New `ouroboros/http_api.py` provides one set of `request_drive_root` / `request_repo_dir` / `coerce_bool` / `coerce_int` / `json_error` helpers; `extensions_api`, `marketplace_api`, `file_browser_api` and `server.py` drop their private copies. UTC ISO timestamp generation routes through `utc_now_iso()` in 9 modules. Governance-doc loading dedup: `_load_bible` and `_load_governance_artifact` thin-wrap `load_governance_doc`. Per-RC: 18 files, +176/-168 = **+8 LOC** (the new helper module's surface area roughly matches the per-call savings; the architectural win is one place to evolve HTTP-API contracts). 3376 tests still green. |
Older releases are preserved in Git tags and GitHub releases. The 5.2.0 through 5.14.0 stable rows (and every 5.x rc tag prior to 5.15.0-rc.2, including v5.15.0-rc.1 which delivered the test-consolidation pass) and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
