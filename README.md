...
[![Version 4.47.1](https://img.shields.io/badge/version-4.47.1-green.svg)](VERSION)
...
- Git
- [uv](https://docs.astral.sh/uv/getting-started/installation/) — fast Python package manager (recommended; `pip install .` works as a fallback)
...
> **Alternative (without uv):** `pip install ".[browser]"` — installs from `pyproject.toml`.
...
Or without uv: `python server.py`.
...
|---------|------|-------------|
| 4.47.1 | 2026-04-23 | **fix(ci): move signing secrets to job-level env for GitHub Actions compatibility.** Step-level `env` vars are not visible to `if:` conditions of the same step in GitHub Actions, causing the workflow to fail with `Unrecognized named-value: 'secrets'`. Signing secrets (`BUILD_CERTIFICATE_BASE64`, `P12_PASSWORD`, `KEYCHAIN_PASSWORD`, `APPLE_TEAM_ID`, `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`) moved to build job-level `env` block. Redundant step-level `env` blocks removed from Import and Build steps. Test `test_ci_uses_env_context_for_condition` now parses full multiline `if` blocks. New `test_ci_signing_secrets_at_job_level` asserts secrets are in the job header. **Note on changelog rolloff**: the v4.42.1 patch entry was rolled off in this release to respect the P7 5-patch-row cap. Its full body remains at git tag `v4.42.1`. |
| 4.47.0 | 2026-04-22 | **feat(ci): optional macOS code signing and notarization in CI.** When Apple signing secrets (`BUILD_CERTIFICATE_BASE64`, `P12_PASSWORD`, `KEYCHAIN_PASSWORD`, `APPLE_TEAM_ID`) are present in GitHub repository secrets, the macOS build job imports the certificate into a temporary keychain, builds a signed `.app` and `.dmg`, and notarizes via `xcrun notarytool` when `APPLE_ID` and `APPLE_APP_SPECIFIC_PASSWORD` are also set. Without secrets, macOS builds remain unsigned as before. `build.sh` parameterized: `SIGN_IDENTITY` accepts env-var override for CI. Keychain is cleaned up unconditionally via `always()` post-step. 14 new tests in `tests/test_build_scripts.py::TestMacOSSigning`. **Note on changelog rolloff**: the v4.42.0 minor entry was rolled off in this release to respect the P7 5-minor-row cap. Its full body remains at git tag `v4.42.0`. |
| 4.46.1 | 2026-04-22 | **fix(tests): sync safety.py with app bundle + align integration tests with v4.44.0 LLMClient.chat() tuple return type.** `ouroboros/safety.py` in git was stuck at the old dual-layer version (v4.18.3) while `test_safety_policy.py` tested the policy-based architecture (v4.40.0+), causing 40+ CI failures. Now synced. `tests/test_provider_integration.py::_assert_basic_response` updated to handle the `(msg, usage)` tuple returned by `LLMClient.chat()` since v4.44.0, with correct provider-first fallback for OpenRouter routing verification. |
| 4.46.0 | 2026-04-22 | **feat(build): migrate from pip to uv as primary package manager.** `pyproject.toml` is now the single source of truth for all dependencies. `requirements.txt` and `requirements-launcher.txt` are removed. `uv.lock` provides deterministic builds for CI and Docker (via `uv sync --frozen`). Build scripts use `uv pip install ".[browser]"` for standalone-Python targets where `uv sync` is not applicable. CI uses `uv sync --frozen --extra browser` + `uv run pytest`. Dockerfile uses `uv sync --frozen --extra browser --no-dev`. Build scripts (`build.sh`, `build_linux.sh`, `build_windows.ps1`) require `uv` and use `uv pip install ".[browser]"` for agent deps into `python-standalone`. Download scripts (`scripts/download_python_standalone.*`) install from `pyproject.toml` via `pip install ".[browser]"`. `Ouroboros.spec` and `MANAGED_BUNDLE_PATHS` now bundle `uv.lock` instead of requirements files. `supervisor/git_ops.py::sync_runtime_dependencies` and `ouroboros/launcher_bootstrap.py::install_deps` install from project metadata with `[browser]` extra. Desktop dependencies moved to `[project.optional-dependencies] desktop`; dev dependencies to `[dependency-groups] dev`. `Makefile` uses `uv run pytest`. New `tests/test_uv_migration.py` with 33 tests verifying the migration. **Note on changelog rolloff**: the v4.40.0 minor entry was rolled off in this release to respect the P7 5-minor-row cap. Its full body remains at git tag `v4.40.0`. |
| 4.45.0 | 2026-04-22 | **feat(chat+ci): clipboard image paste, autocorrect disable, provider integration tests.** Chat textarea gains `autocorrect="off" autocapitalize="off" spellcheck="false"` to suppress macOS smart-text interference. Cmd+V / Ctrl+V now stages clipboard images as attachments (same pipeline as the paperclip button — no upload until Send). New `tests/test_provider_integration.py` with 6 `@pytest.mark.integration` tests (OpenRouter, OpenAI, Anthropic — basic + isolation) excluded from default `pytest` via `addopts`; new CI `integration-test` job runs on `ouroboros-stable`, `workflow_dispatch`, and `v*` tags; `build` job now gates on `[full-test, integration-test]`. ~160 lines of new code. **Note on changelog rolloff**: the v4.41.0 minor entry was rolled off in this release to respect the P7 5-minor-row cap. Its full body remains at git tag `v4.41.0`. |
