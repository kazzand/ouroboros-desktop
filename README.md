...
[![Version 4.40.2](https://img.shields.io/badge/version-4.40.2-green.svg)](VERSION)
...
|---------|------|-------------|
| 4.40.2 | 2026-04-19 | **Chromium bundled in all platform builds.** `build.sh`, `build_linux.sh`, and `build_windows.ps1` now run `PLAYWRIGHT_BROWSERS_PATH=0 playwright install chromium` (inside `python-standalone`) before PyInstaller, so the Chromium binary ships inside every `.app`/`.tar.gz`/`.zip` bundle — browser tools (`browse_page`, `browser_action`, `analyze_screenshot`) work out of the box for all new users without a first-run download. Linux caveat: some hosts may still need native system libraries; use the bundled Python: `./Ouroboros/python-standalone/bin/python3 -m playwright install-deps chromium` (the bare `playwright` command is not on PATH in packaged installs). `browser.py` updated: new `_set_playwright_browsers_path_if_bundled()` sets `PLAYWRIGHT_BROWSERS_PATH=0` at module import time **only** when `_has_platform_chromium()` confirms a platform-matching `chromium-*` entry in `.local-browsers/` (two-level check prevents false positives from foreign-platform payloads; preserves standard `~/.cache/ms-playwright/` lookup for source/dev installs); `_chromium_is_present()` fast-path is now arm64-alias-aware; removed dead `sys.frozen` guard (worker processes are never frozen); removed `install-deps` auto-call (hangs on Linux without a TTY); wired `emit_progress` for source/dev first-run feedback. 26 new regression tests: `tests/test_browser_isolation.py` adds `TestSetPlaywrightBrowsersPathIfBundled` (5 tests: pre-set env, chromium-dir sets 0, non-chromium dir leaves unset, empty dir, missing dir) + `TestEnsurePlaywrightInstalled` (6 tests); `tests/test_build_scripts.py` adds 15 tests (3 per platform × 3 build scripts + 6 for Dockerfile) asserting `playwright install chromium` presence, `PLAYWRIGHT_BROWSERS_PATH=0`, `playwright install-deps chromium` presence, and correct install ordering. Also updates `Dockerfile`: uses `playwright install-deps chromium` (authoritative Playwright dependency resolver, not a hand-curated apt list) **and** `PLAYWRIGHT_BROWSERS_PATH=0 python3 -m playwright install chromium` so browser tools work in Docker out of the box. |
