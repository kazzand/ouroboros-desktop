...
[![Version 5.3.6-rc.3](https://img.shields.io/badge/version-5.3.6--rc.3-green.svg)](VERSION)
...
|---------|------|-------------|
| 5.3.6-rc.3 | 2026-04-29 | **pre-release: package the hardened skills/extensions platform for cross-platform validation.** This release candidate packages the v5.3.x extension/skill improvements currently on `ouroboros`: trust-state hardening, QA UI repairs, extension unload lifecycle hooks, extension push events, host-rendered widget v2 components, and the constrained Heal/Fix flow. It is intended for GitHub Actions build verification and prerelease artifact distribution without merging to `main`; prerelease macOS artifacts are intentionally unsigned so signing/notarization issues do not block RC artifact validation. **Note on changelog rolloff**: the v5.3.1 patch entry was rolled off in this pre-release to respect the P7 changelog cap. Its full body remains at git tag `v5.3.1`. |
