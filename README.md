...
[![Version 4.40.5](https://img.shields.io/badge/version-4.40.5-green.svg)](VERSION)
...
| 4.40.2 | 2026-04-19 | **Chromium bundled in all platform builds — Dockerfile and test infrastructure.** `Dockerfile` updated to use `playwright install-deps chromium` (authoritative Playwright dependency resolver) and `PLAYWRIGHT_BROWSERS_PATH=0 python3 -m playwright install chromium` so browser tools work out of the box in Docker. `tests/test_build_scripts.py` adds 15 tests (3 per platform × 3 build scripts + 6 for Dockerfile) asserting `playwright install chromium` presence, `PLAYWRIGHT_BROWSERS_PATH=0`, `playwright install-deps chromium` presence, and correct install ordering. `docs/ARCHITECTURE.md` and `README.md` document the bundled-Chromium strategy. Build-script install steps and `browser.py` runtime detection shipped in v4.40.3. |
