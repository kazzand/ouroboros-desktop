...
[![Version 5.8.0-rc.7](https://img.shields.io/badge/version-5.8.0--rc.7-green.svg)](VERSION)
...
### Integrations: A2A vs MCP

Ouroboros exposes two complementary integration lanes in **Settings → Integrations**:

| | **A2A (Agent-to-Agent)** | **MCP (Model Context Protocol)** |
|---|---|---|
| Purpose | Talk *to* other agents | Borrow *tools* from external services |
| Direction | Server (others call us) | Client (we call them) |
| Default | Disabled, restart-required | Disabled, hot-reloadable |
| Settings | `A2A_*` keys | `MCP_ENABLED`, `MCP_TOOL_TIMEOUT_SEC`, `MCP_SERVERS` (list) |
| Tool surface | `a2a_discover` / `a2a_send` / `a2a_status` (built-ins) | Discovered MCP tools registered as `mcp_<server>__<tool>` |

Each MCP server entry is a small dict (`id`, `name`, `enabled`, `transport`, `url`, `auth_header`, `auth_token`, `allowed_tools`). The widget supports `streamable_http` and `sse` transports. Auth tokens are masked on the wire and rehydrated from the on-disk old settings on save, so a UI round-trip without re-typing the secret cannot overwrite the working credential.

MCP tools are non-core by default — discover them via the agent's `list_available_tools` / `enable_tools` flow.

The HTTP surface used by the UI is `GET /api/mcp/status`, `POST /api/mcp/refresh`, `POST /api/mcp/test`. The Python `mcp` SDK is an optional runtime dependency declared in `requirements.txt`; missing-SDK installs degrade gracefully (`sdk_available=false` in the status payload) and the rest of the app keeps working.
...
|---------|------|-------------|
| 5.8.0-rc.7 | 2026-05-07 | **feat(integrations): add MCP (Model Context Protocol) client.** Settings → Integrations now ships a multi-server MCP card widget next to A2A. Each server entry persists in `MCP_SERVERS` with hot-reloadable hot-swap on `/api/settings` save, masked `auth_token` on the GET surface, rehydration from the on-disk old settings on POST so the user doesn't have to re-type the credential, and a URL deny-list that refuses cloud-metadata endpoints (`169.254.169.254`, `metadata.google.internal`, etc.) and IPv4 link-local addresses. Discovered tools surface through the existing `ToolRegistry` as `mcp_<server>__<tool>`, are non-core by default (require `enable_tools`), reach the standard `safety.check_safety` recheck before dispatch, and are explicitly blocked in heal-mode contexts. New endpoints: `GET /api/mcp/status`, `POST /api/mcp/refresh`, `POST /api/mcp/test`. The Python `mcp` SDK is an optional runtime dependency — missing-SDK installs degrade gracefully (`sdk_available=false` in the status payload) and the rest of the app keeps working. |
...
Older releases are preserved in Git tags and GitHub releases. The 5.2.0, 5.3.0, 5.3.x release-candidate, 5.4.0, 5.5.0, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
