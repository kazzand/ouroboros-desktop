...
|---------|------|-------------|
| 4.50.0-rc.9 | 2026-04-25 | **feat(runtime-mode): direct pro core-patch gate + gpt-5.5 review defaults.** `advanced` now preserves normal evolutionary self-modification while blocking protected core/contract/release surfaces through a shared `ouroboros/runtime_mode_policy.py` policy (`BIBLE.md`, safety files, `ouroboros/contracts/`, release/managed invariants such as `.github/workflows/ci.yml`, build scripts, `scripts/build_repo_bundle.py`, `ouroboros/launcher_bootstrap.py`, `supervisor/git_ops.py`). `pro` can edit those protected paths on disk, but `repo_commit` must pass the extra `ouroboros/tools/core_patch_gate.py::run_core_patch_review_gate` before the protected diff can land; skipped/incomplete core-patch scope review now blocks instead of counting as pass. `claude_code_edit`, `repo_write`, `str_replace_editor`, and staged commit paths all use the same policy and emit `CORE_PATCH_NOTICE` for pro protected edits. Review defaults move to the GPT-5.5 family: commit triad default starts with `openai/gpt-5.5`, scope review default is `openai/gpt-5.5`, and deep self-review uses `openai/gpt-5.5-pro` / `openai::gpt-5.5-pro`; UI placeholders/docs/tests are synchronized. Review context hygiene improved: `build_full_repo_pack` redacts inline secret-shaped values, scope and plan review inject canonical docs (`BIBLE.md`, `DEVELOPMENT.md`, `ARCHITECTURE.md`, `CHECKLISTS.md`) exactly once, and touched tests/canonical docs are represented via staged diff plus an explicit deduplication note instead of duplicated full-file snapshots. Managed-repo safety improved by blocking `rescue_and_reset` when rescue snapshot/diff capture fails, and CI quick-test now also triggers for `ouroboros-three-layer`. No GitHub prerelease is published by user request; the annotated tag can still be created later if this commit is promoted into a packaged prerelease. **Note on changelog rolloff**: the v4.49.0 minor entry is rolled off in this release to respect the P7 5-minor-row cap. Its full body remains at git tag `v4.49.0`. |
