...
[![Version 4.28.6](https://img.shields.io/badge/version-4.28.6-green.svg)](VERSION)
...
|---------|------|-------------|
| 4.28.6 | 2026-04-12 | Checkpoint hardening: structural tool suppression (`tools=None`) on checkpoint rounds forces text-only reflection instead of relying on prompt instructions; compaction skipped on checkpoint rounds (including pending) to preserve full context; `_handle_checkpoint_response` helper extracted from `run_llm_loop` (keeps method under 250-line gate); anchored `startswith` marker detection prevents false-positive continuation; reflection events emitted only when marker present; continuation logic — if `CHECKPOINT_REFLECTION:` header present, loop continues with tools on next round; else treats as final answer. 6 new regression tests in `TestCheckpointToolsNone`. P3 reconciliation note added to `docs/DEVELOPMENT.md`. |
