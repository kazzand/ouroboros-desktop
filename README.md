...
[![Version 4.42.0](https://img.shields.io/badge/version-4.42.0-green.svg)](VERSION)
...
| 4.41.1 | 2026-04-20 | **refactor(preflight): shared test runner for advisory + bypass paths.** Extracts `_run_review_preflight_tests` into `review_helpers.py` as the canonical shared test runner. `_run_advisory_tests` in `claude_advisory_review.py` becomes a thin wrapper (eliminates duplication). `_run_reviewed_stage_cycle` in `git.py` now calls `_run_review_preflight_tests` when `skip_advisory_pre_review=True` (or no Anthropic key), blocking before triad+scope review if tests fail — previously broken code could reach the expensive LLM review when advisory was bypassed. Both call sites share the same `OUROBOROS_PRE_PUSH_TESTS=0` kill switch. 4 new regression tests in `TestBypassPathTestsRun`. |
| 4.41.0 | 2026-04-20 | **feat(preflight): Level A preflight hardening — P7 history limits in commit preflight + test-first advisory.** (1) `_preflight_check` check 7 (previously reserved): when VERSION is staged, reads staged `README.md` via `git show :README.md` and calls `check_history_limit()` from `release_sync.py` — blocks with `⚠️ PREFLIGHT_BLOCKED` when Version History exceeds P7 limits (2 major / 5 minor / 5 patch rows). Deterministic, no LLM call. (2) `advisory_pre_review` gains `skip_tests: bool = False` parameter: by default runs `pytest tests/ -q` before the expensive Claude SDK call — if tests fail, returns `status="tests_preflight_blocked"` with the failure output, saving ~$1.3 SDK spend on broken code. Use `skip_tests=True` for intentional WIP. 15 new regression tests. |
...
| 4.40.3 *(rolled off — see git tag)* | 2026-04-19 | Fix CI: playwright install + bundled browser detection. Full body at git tag `v4.40.3`. |
