...
[![Version 5.15.0-rc.8](https://img.shields.io/badge/version-5.15.0--rc.8-green.svg)](VERSION)
...
| 5.15.0-rc.5 | 2026-05-10 | **refactor(tests): final stretch pass + honest accounting before pre-release tag.** Adds `drive_root` fixture in `tests/test_advisory_workflow.py` collapsing 9 paired `_make_drive_root(tmp_path)` setups; fixes `test_marketplace_ui_static.py` after rc.4's `renderSkillRepairPrompt` extraction. |
| 5.15.0-rc.4 | 2026-05-10 | **refactor(web): consolidate skill-repair prompt + lifecycle event helper + alert→toast SSOT.** Web/JS dedup pass. `web/modules/utils.js` gains `emitSkillLifecycle()` and `renderSkillRepairPrompt(intro, diagnosticsJson)` — single sources of truth for the `ouro:skill-lifecycle` CustomEvent and for the My Skills / ClawHub Marketplace heal-prompt narrative. Replaced 9 native `alert()` calls with `showToast()`. |
Older releases are preserved in Git tags and GitHub releases. The 5.2.0 through 5.14.0 stable rows (and every 5.x rc tag prior to 5.15.0-rc.4, including v5.15.0-rc.1 through v5.15.0-rc.3) and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
