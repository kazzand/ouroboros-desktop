...
[![Version 5.15.0-rc.5](https://img.shields.io/badge/version-5.15.0--rc.5-green.svg)](VERSION)
...
|---------|------|-------------|
| 5.15.0-rc.5 | 2026-05-10 | **refactor(tests): final stretch pass + honest accounting before pre-release tag.** Adds `drive_root` fixture in `tests/test_advisory_workflow.py` collapsing 9 paired `_make_drive_root(tmp_path)` setups; fixes `test_marketplace_ui_static.py` after rc.4's `renderSkillRepairPrompt` extraction. Cumulative v5.14.0 → v5.15.0-rc.5: roughly **−445 LOC** across 49 files (315 inserts, 760 deletes), 3373 tests still green. The original plan budgeted −8000 LOC; the realistic ceiling under the chosen MODERATE envelope (no feature-moves-to-skills, no high-blast-radius rewrites like `repo_write_commit` collapse / `review_state.py` compat ledger trim / `tools/git.py` broad-except dedup) was ~1500-2500 LOC per audit. Hitting -8000 would require either AGGRESSIVE scope (extract `claude_code_gateway`, `vision`, `web_search` to OuroborosHub skills) or accepting medium-to-high blast-radius rewrites. Tagged as `v5.15.0-rc.5` pre-release; CI auto-detects the rc suffix and ships notarized artifacts. |
