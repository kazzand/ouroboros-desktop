...
|---------|------|-------------|
| 5.8.0-rc.3 | 2026-05-06 | **fix(skills): collapse lifecycle work into one live chat card.** Skill review/install/update progress now uses the existing lifecycle `JobProgressTarget` end to end, including dependency reconciliation and extension reload, while the old per-kind markdown summaries and fixed `api_skill_review` summary bubble are removed so review/repair flows do not leave orphan running cards. The release also verifies OuroborosHub DuckDuckGo dependency metadata (`joi-lab/OuroborosHub@e7c979b`), declaring `ddgs` in both the skill manifest and catalog `install_specs` so reviewed installs can auto-install the package. |
