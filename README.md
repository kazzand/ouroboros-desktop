...
[![Version 5.15.0-rc.7](https://img.shields.io/badge/version-5.15.0--rc.7-green.svg)](VERSION)
...
|---------|------|-------------|
| 5.15.0-rc.7 | 2026-05-10 | **fix(security): sanitise untrusted diagnostics inside `renderSkillRepairPrompt` SSOT helper.** Round-2 cumulative real triad/scope review on the v5.15.0-rc.6 diff caught a critical prompt-injection risk: `web/modules/utils.js::renderSkillRepairPrompt` wraps untrusted `diagnosticsJson` inside a Markdown ``` ```json ``` fence, and a diagnostic string containing literal triple-backticks (e.g. inside a `load_error` or `review_findings.reason` captured from external skill/reviewer text) could close the fence and inject instructions into the prompt outside the data block. The pre-rc.4 marketplace.js code had a caller-local `JSON.stringify(...).replace(/`/g, "'")` workaround; the pre-rc.4 skills.js code had no sanitisation at all; rc.4's helper extraction kept the marketplace caller's escape but left skills.js exposed. rc.7 moves the sanitisation INTO the SSOT helper (`String(diagnosticsJson ?? '').replace(/```/g, "'''").replace(/`/g, "'")`) so every caller inherits the safety invariant for free, and removes the now-redundant marketplace.js caller-local escape. Also fixes one stale `docs/ARCHITECTURE.md` Reasoning Effort bullet (left a contradictory "Loading falls back to legacy" prefix). 3373 tests still green. |
| 5.15.0-rc.6 | 2026-05-10 | **chore(release): correct accounting and structural docs after multi-model review.** Fixes blockers raised by the cumulative `/adversarial-multimodel-review` (gemini-critic + gpt-critic + opus-critic) and the real triad/scope review on the v5.15.0-rc.5 cumulative diff. (1) Corrects every LOC number quoted in this README and in the per-RC commit bodies — `git diff --shortstat v5.14.0..v5.15.0-rc.6` now reads `48 files changed, 824 insertions(+), 1196 deletions(-)` net **−372 LOC** (was advertised as `−445 LOC, 49 files, 315/760` — wrong on three fields and on per-RC subtotals; the rc.4 row was also reported with a sign flip). (2) Bumps `docs/ARCHITECTURE.md` header from `v5.14.0` to current to clear the preflight version-sync block. (3) Adds the new `ouroboros/http_api.py` module to the `docs/ARCHITECTURE.md` structural map (P6 violation — every structural change must update the operational map in the same commit). (4) Updates the stale `skill_migrations.py` line in the structural map after rc.3 retired the generation-skill rename. (5) Documents the deliberate behavior of the rc.4 web changes that the critics flagged: `renderSkillRepairPrompt` deliberately adopts the longer skills.js untrusted-data narrative (a strict improvement to the marketplace surface), and the two surviving `alert()` calls in the `settings.js` Reset handler are intentional (modal acknowledgment before server restart). Tests still green. |
| 5.15.0-rc.5 | 2026-05-10 | **refactor(tests): final stretch pass + honest accounting before pre-release tag.** Adds `drive_root` fixture in `tests/test_advisory_workflow.py` collapsing 9 paired `_make_drive_root(tmp_path)` setups; fixes `test_marketplace_ui_static.py` after rc.4's `renderSkillRepairPrompt` extraction. Per-RC: 5 files, +28/-25 = **+3 LOC**. |
| 5.15.0-rc.4 | 2026-05-10 | **refactor(web): consolidate skill-repair prompt + lifecycle event helper + alert→toast SSOT.** Web/JS dedup pass. `web/modules/utils.js` gains `emitSkillLifecycle()` and `renderSkillRepairPrompt(intro, diagnosticsJson)` — single sources of truth for the `ouro:skill-lifecycle` CustomEvent (was duplicated in `marketplace.js` and `ouroboroshub.js`) and for the My Skills / ClawHub Marketplace heal-prompt narrative. Replaced 9 native `alert()` calls in `settings.js`, `updates.js`, `settings_local_model.js`, and `chat.js` with `showToast()`. Per-RC: 12 files, +93/-71 = **+22 LOC** (new shared helpers exceed per-call savings; the architectural win is one source of truth for both the lifecycle CustomEvent and the heal-prompt narrative, plus completing the v5.13.0 notification SSOT migration). |
| 5.15.0-rc.3 | 2026-05-10 | **refactor(legacy): retire pre-v5.5 / pre-v5.0 compatibility shims.** Removes `migrate_generation_skill_names` + `_RENAME_SPECS` + `_rewrite_text_files` + `_looks_like_known_legacy_skill` from `skill_migrations.py`; the `migrate_unseeded_native_skills_to_external` topology repair stays. Retires the `OUROBOROS_INITIAL_REASONING_EFFORT` legacy alias from `config.resolve_effort` + the dead read in `web/modules/settings.js` + tests. Deletes the unused `get_clawhub_enabled` helper and the `OUROBOROS_CLAWHUB_ENABLED` env var plus the dead `enable_marketplace` fixture. Per-RC: 14 files, +44/-206 = **−162 LOC**; 3373 tests still green (3 obsolete legacy tests removed). |
Older releases are preserved in Git tags and GitHub releases. The 5.2.0 through 5.14.0 stable rows (and every 5.x rc tag prior to 5.15.0-rc.3, including v5.15.0-rc.1 which delivered the test-consolidation pass and v5.15.0-rc.2 which centralised HTTP-API plumbing + UTC + governance dedup) and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
