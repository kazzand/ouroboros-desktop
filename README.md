...
[![Version 4.35.1](https://img.shields.io/badge/version-4.35.1-green.svg)](VERSION)
...
| 4.32.1 | 2026-04-16 | Unified review input budget at 850K tokens across `scope_review.py` (`_SCOPE_BUDGET_TOKEN_LIMIT`: 750K→850K), `plan_review.py` (`_PLAN_BUDGET_TOKEN_LIMIT`: 1M→850K), and `deep_self_review.py` pack gate (900K→850K). Three alignment fixes: (1) `deep_self_review.py` switched from its local `chars/3.5` estimator to the shared `ouroboros.utils.estimate_tokens` (chars/4) so the effective char budget is identical across all three surfaces. (2) `deep_self_review.py` gate now evaluates the FULL assembled prompt (`_SYSTEM_PROMPT + pack_text`), matching how scope/plan gate the assembled prompt; gating only on `pack_text` understated the real request size. (3) Documentation honesty: comments in `scope_review.py`, `plan_review.py`, `deep_self_review.py`, and `ARCHITECTURE.md` now correctly state that 1M-context reviewer windows are **shared** between input and output (not separate budgets); scope/deep comments scope the claim to their actual default models (`anthropic/claude-opus-4.6` for scope, `openai/gpt-5.4-pro` for deep) while plan_review describes 850K as a best-effort shared policy because `plan_task` uses the configurable `OUROBOROS_REVIEW_MODELS` set with varying context windows. At gate=850K (~1M actual input with chars/4 undercount), near-gate prompts sit tight to the API ceiling — the skip path is best-effort, and 850K is a conscious trade that lets scope prompts which previously skipped at ~778K actually run. The README `/review` command docstring was updated to list the full memory-whitelist contract (identity, scratchpad, registry, WORLD, knowledge index, patterns, improvement-backlog). Tests in `test_max_tokens_constants.py` and `test_deep_self_review.py` updated to pin the unified 850K value, the shared estimator, and the full-assembled-prompt gate. Docs in `ARCHITECTURE.md` updated across all three review sections. |
