...
[![Version 5.8.0-rc.4](https://img.shields.io/badge/version-5.8.0--rc.4-green.svg)](VERSION)
...
|---------|------|-------------|
| 5.8.0-rc.4 | 2026-05-06 | **fix(mobile): keep keyboard touch locks scoped to mobile Chat surfaces.** The keyboard-open touch boundary now treats the transcript, multiline composer textarea, and live-task timeline as legitimate scrollable surfaces, so long messages and expanded task cards remain usable while page-level overscroll is still blocked. The listener cleanup path also removes touch handlers when a device rotates or resizes out of the narrow mobile viewport while the keyboard is open. **Note on changelog rolloff**: the v5.5.0 minor row was rolled off to respect the P9 5-minor-row cap; its full body remains at git tag `v5.5.0`. |
