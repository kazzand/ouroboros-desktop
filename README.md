...
[![Version 5.8.0-rc.5](https://img.shields.io/badge/version-5.8.0--rc.5-green.svg)](VERSION)
...
|---------|------|-------------|
| 5.8.0-rc.5 | 2026-05-06 | **fix(mobile): make keyboard touch-lock guards precise.** The mobile keyboard touch-boundary helper now tolerates text-node touch targets with optional `classList` access, and its regression test checks the wide-viewport listener cleanup branch directly instead of matching the older narrow-keyboard cleanup path. This preserves scrollability for the transcript, composer textarea, and live-task timeline while keeping page-level overscroll blocked. **Note on changelog rolloff**: the v5.6.0 minor row was rolled off to respect the P9 5-minor-row cap; its full body remains at git tag `v5.6.0`. |
| 5.8.0-rc.4 | 2026-05-06 | **fix(mobile): keep keyboard touch locks scoped to mobile Chat surfaces.** The keyboard-open touch boundary now treats the transcript, multiline composer textarea, and live-task timeline as legitimate scrollable surfaces, so long messages and expanded task cards remain usable while page-level overscroll is still blocked. The listener cleanup path also removes touch handlers when a device rotates or resizes out of the narrow mobile viewport while the keyboard is open. |
| 5.8.0-rc.3 | 2026-05-06 | **fix(mobile): harden chat keyboard layout against visualViewport overscroll.** The mobile Chat page now uses a frozen visual-viewport baseline for iOS/Android keyboard detection, constrains `html`/`body`/`#content` to `--vvh` while the keyboard is open, pins only the active Chat page as a fixed flex-column stack, and keeps `#chat-messages` as the sole scroll container. Boundary-aware touch handling allows normal transcript scrolling but stops top/bottom overscroll from chaining into page or visualViewport movement, preventing the blank gaps reproduced on iOS Safari and Android Opera. |
