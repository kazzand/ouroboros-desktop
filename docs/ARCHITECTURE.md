# Ouroboros v4.28.1 — Architecture & Reference
...
- **Live card retirement and reconnect recovery**: finished task cards are removed from the DOM after a 2-minute `scheduleTaskUiCleanup` timer and their task IDs added to `retiredTaskIds`. On soft restart (same-SHA WS reconnect), `syncHistory` receives `fromReconnect=true` so `retiredTaskIds` is cleared and server history is authoritative — retired cards are reconstructed from `progress.jsonl` entries. Routine `scheduleHistorySync()` calls (fired 700ms after task completion) do NOT pass `fromReconnect`, so they never resurrect cards mid-session.
