# Ouroboros v4.43.0 — Architecture & Reference
...
- **Mobile keyboard safety**: `web/app.js` attaches a `visualViewport` resize/scroll listener at startup that writes the real visible-area height into `--vvh` CSS custom property on `document.documentElement`. `body`, `#app`, and `#nav-rail` use `height: var(--vvh)` instead of `100vh`, keeping the chat input above the soft keyboard on iOS/Android. `--vvh` defaults to `100dvh` in `:root` so desktop layout is unchanged. `renderMarkdown` wraps generated tables in `<div class="md-table-wrap">` for `overflow-x: auto` horizontal scrolling on narrow screens.
