...
Uses Starlette TestClient against the real ``server.app``. The MCP
manager is wired with a fake transport so no live MCP server is needed.
"""
...
def _make_client(tmp_path, monkeypatch):
    ...
    for p in patches:
        p.start()
    return TestClient(srv.app), patches
