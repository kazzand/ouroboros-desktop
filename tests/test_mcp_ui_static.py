...
def test_settings_ui_mcp_section_sits_in_integrations_panel(settings_ui_source: str) -> None:
    """The MCP Servers section must render inside the Integrations
    panel (next to A2A), not somewhere else like Advanced or Behavior."""
    integrations_index = settings_ui_source.find('data-settings-panel="integrations"')
    advanced_index = settings_ui_source.find('data-settings-panel="advanced"')
    mcp_index = settings_ui_source.find("<h3>MCP Servers</h3>")
    assert integrations_index >= 0 and advanced_index >= 0 and mcp_index >= 0
    assert integrations_index < mcp_index < advanced_index
...
def test_mcp_module_escapes_untrusted_strings(mcp_source: str) -> None:
    ...
    assert "function escapeHtml" in mcp_source
